    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        ...
        <div class="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg hover:shadow-xl ring-1 ring-white/10 transition-all duration-300">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="text-4xl font-bold text-gray-800"><?php echo $kelas->bahasa_program ?? 'Umum'; ?></h3>
        ...
        <div class="bg-white/80 backdrop-blur-lg p-6 rounded-2xl shadow-lg hover:shadow-xl ring-1 ring-white/10 transition-all duration-300">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="text-4xl font-bold text-gray-800"><?php echo $kelas->level ?? 'Dasar'; ?></h3>
            ...
            <?php else: ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <?php foreach ($materi as $m): ?>
                        <?php
                        $m_type = $kelas->type ?? 'premium';
                        $detail_url = $m_type === 'free'
                            ? site_url('admin/free_classes/edit_material/' . $m->id)  // Adjust as needed
                            : site_url('materi/detail/' . $m->id);
                        $edit_url = $m_type === 'free'
                            ? site_url('admin/free_classes/edit_material/' . $m->id)
                            : site_url('materi/edit/' . $m->id);
                        $delete_url = $m_type === 'free'
                            ? site_url('admin/free_classes/delete_material/' . $m->id)
                            : site_url('materi/delete/' . $m->id);
                        ?>
                        <div class="group relative bg-white rounded-2xl shadow-lg hover:shadow-xl overflow-hidden border border-gray-200/50 transition-all duration-300 transform hover:-translate-y-1">
                            ...
                            <div class="p-6 relative z-10">
                                <div class="flex justify-between items-start mb-4">
                                    ...
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-<?php echo $m_type === 'free' ? 'green' : 'blue'; ?>-100 text-<?php echo $m_type === 'free' ? 'green' : 'blue'; ?>-800">
