<div class="p-4 transition-opacity duration-500 opacity-0">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 bg-white p-6 rounded-2xl shadow-xl ring-1 ring-gray-200/50">
        <div class="mb-4 md:mb-0">
            ...
            <p class="text-lg text-gray-500 mt-2">Buat akun user baru dengan role dan level yang sesuai</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-xl ring-1 ring-gray-200/50 overflow-hidden mb-8">
        ...
        <div class="p-6">
            <?php echo form_open('admin/users/create', 'class="space-y-6"'); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="password" id="password" name="password" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-2">
                        ...
                        <select id="role" name="role" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required onchange="updateLevel()">
                            ...
                            <option value="guru" <?php echo set_select('role', 'guru'); ?>>Guru</option>
                            <option value="siswa" <?php echo set_select('role', 'siswa'); ?>>Siswa</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <select id="level" name="level" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                            ...
                            <option value="1" <?php echo set_select('level', '1'); ?>>Level 1 (Super Admin)</option>
                            <option value="2" <?php echo set_select('level', '2'); ?>>Level 2 (Admin)</option>
                            <option value="3" <?php echo set_select('level', '3'); ?>>Level 3 (Guru)</option>
                            <option value="4" <?php echo set_select('level', '4'); ?>>Level 4 (Siswa)</option>
                            <option value="5" <?php echo set_select('level', '5'); ?>>Level 5 (User)</option>
                        </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" id="department" name="department" class="block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" 
                           value="<?php echo set_value('department'); ?>" placeholder="Contoh: IT, Marketing, HR">
    ...
    <div class="bg-white rounded-2xl shadow-xl ring-1 ring-gray-200/50 overflow-hidden">
        <div class="p-6 border-b border-gray-200/50">
            <h2 class="text-2xl font-bold text-gray-800">Panduan Role & Level</h2>
        </div>
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                <div class="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-red-100 text-red-800">Super Admin</span>
                        <span class="px-2 py-1 text-xs font-medium rounded-full border border-gray-300 text-gray-700">Level 1</span>
                    </div>
                    <p class="text-sm text-gray-600">Akses penuh sistem, kelola user, permissions, dan semua modul</p>
                </div>
                <div class="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-yellow-100 text-yellow-800">Admin</span>
                        <span class="px-2 py-1 text-xs font-medium rounded-full border border-gray-300 text-gray-700">Level 2</span>
                    </div>
                    <p class="text-sm text-gray-600">Kelola siswa, kelas, materi, dan guru (tanpa user management)</p>
                </div>
                <div class="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800">Guru</span>
                        <span class="px-2 py-1 text-xs font-medium rounded-full border border-gray-300 text-gray-700">Level 3</span>
                    </div>
                    <p class="text-sm text-gray-600">Akses kelas yang ditugaskan, siswa di kelas, dan materi</p>
                </div>
                <div class="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800">Siswa</span>
                        <span class="px-2 py-1 text-xs font-medium rounded-full border border-gray-300 text-gray-700">Level 4</span>
                    </div>
                    <p class="text-sm text-gray-600">Akses profil sendiri dan materi pembelajaran untuk kelasnya</p>
                </div>
                <div class="p-4 border border-gray-200 rounded-xl hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2 py-1 text-xs font-medium rounded-full bg-gray-100 text-gray-800">User</span>
                        <span class="px-2 py-1 text-xs font-medium rounded-full border border-gray-300 text-gray-700">Level 5</span>
                    </div>
                    <p class="text-sm text-gray-600">Akses terbatas, untuk keperluan khusus atau testing</p>
                </div>
            </div>
        </div>
    </div>
